...
;; Adaptado para carregar BM.BIN em 0:8000h
;; 
...
bootloader_start:
	...
	;;==================================
	;;incluindo esse ajuste de segmentos.
	;;para refletir o ambiente de aplicativos do mikeos.
    ...
    ;;#importante
	;;cancelando o ajuste de pilha que tinha no mbr antigo.
	
	;mov ax, 0x2000 ;07C0h			; Set up 4K of stack space above buffer
	...
	;; #importante
	;; Colocando no ds o segmento onde estamos, ou seja, o ambiente 
	;; onde rodam os aplicativos do mikeos.
	;; ; Set data segment to where we're loaded
	
	
	;;
	;; ds em  07C0h  (isso significa que o org está em 0.)
	;;
	
	mov ax, 0x2000 ;07C0h			; Set data segment to where we're loaded
	mov ds, ax
...
floppy_ok:				; Ready to read first block of data
	...
	;;#importante
	;;ok o buffer dica no fim desse arquivo
	;;se ele lidar com esse segmento, então tudo vai dar certo.
	...
	;;#importante 
	;;agora es também estará em 2000h que é o ambiente dos aplicativos 
	;;do mikeos, onde estamos.
...
search_dir:
	...
	;;#importante 
	;;continuamos no ambiente dos aplicativos do mikeos
...
found_file_to_load:			; Fetch cluster and load FAT into RAM
	...
	;;#importante 
	;; es, é o mesmo que ds, = 2000h, ambiente dos aplicativos do mikeos.
	;;
	...
	;;o buffer esta no fim desse arquvo, em 2000h:buffer
...
read_fat_ok:
	...
	;;#importante 
	;; es = 0.
	;; colocaremos 0 em es porque é onde desejamos que BM.BIN seja carregado.
	;; bx = 8000h é o offset da imagem ...
	;;; >>>> 0:8000h é aqui que desejamos que o trem fique.
	...
	mov ax, 0 ;;2000h			; Segment where we'll load the kernel
	mov es, ax
	mov bx, 0x8000  ; 0
...
load_file_sector:
	...
	mov ax, 0 ;;2000h			; Set buffer past what we've already read
	mov es, ax
	mov bx, word [pointer]
...
; STRINGS AND VARIABLES
	...
	pointer		dw 0x8000 	; Pointer into Buffer, for loading kernel
